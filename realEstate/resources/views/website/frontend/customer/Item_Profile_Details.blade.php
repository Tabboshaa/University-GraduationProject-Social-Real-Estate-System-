...
@section('profile_Content')
    ...
    <script
        src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBCUywuD0K3ICLer31HgVIJ-Uhi_Suj2jA&callback=initMap&libraries=&v=weekly"
        async
    ></script>
    ...
    <div class="card w-100 shadow-xss rounded-xxl border-0 mb-3">
        ...
        @foreach($schedule as $year => $schedules)
            ...
            @foreach($schedules as $s)
                ...
                $date = \Carbon\Carbon::createFromFormat('m/d/Y', $test);
                ...
                <div class="card-body d-flex pt-0 ps-4 pe-4 pb-3 overflow-hidden">
                    <div class="bg-skype me-2 p-3 rounded-xxl w125">
                        <a href="javascript:void(0)" id="" onclick="$('#ul').slideToggle();"></a>
                        <h4 class="fw-700 font-lg ls-3 lh-1 text-white mb-0"><span class="ls-1 d-block font-xsss text-white fw-600 text-center">{{$monthName}}</span></h4>
                    </div>
                </div>
                        ...
                        <h4 class="fw-700 text-grey-900 font-xssss mt-2">
                            ...
                            <li>
                                <div> <span id="" name="" class="calendar-table__item" href="javascript:void(0)" onclick="" style="color: #0C0C0C">{{$day}}</span></div>
                            </li>
                                @endforeach
                            </ul>
                        </h4>
                                ...
                                @if(count($detailValue) !=0 )
                                    @foreach ($detailValue as $detail)
                                        ...
                                        @elseif($detail->datatype=='file')
                                            <li>{{$detail->Detail_Name}} : <div class="col-6 mb-2 pe-1"><a href="{{$detail->DetailValue}}" data-lightbox="roadtrip"><img src="{{asset('storage/profile gallery/'.$detail->DetailValue)}}" alt="image" class="img-fluid rounded-3 w-100"></a></div></li>
                                        @endif
                                    @endforeach
                                @else
