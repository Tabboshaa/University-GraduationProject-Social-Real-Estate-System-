...
<div class="row">
    ...
    <div class="col-xl-8 col-xxl-9 col-lg-8">
        ...
        <div class="card w-100 shadow-xss rounded-xxl border-0 ps-4 pt-4 pe-4 pb-3 mb-3 mt-3">
            <form method="post" action="{{url('/add_post')}}" id="postform" enctype="multipart/form-data">
                ...
                <div class="card-body d-flex p-2 mt-0">
                    ...
                    <input type="file" style="display:none;" id="uploadImagespost" name="images[]" accept="image/*" placeholder="upload Images" multiple>
        ...
        <div class="card w-100 shadow-xss rounded-xl border-0 p-4 mb-3">
            ...
            <a href="javascript:void(0)" id="more{{$post->Post_Id}}" onclick="$('#allcomments{{$post->Post_Id}}').slideToggle(function(){$('#more{{$post->Post_Id}}').html($('#allcomments{{$post->Post_Id}}').is(':visible')?'Hide Comments':'<i class=\'feather-message-circle text-dark text-grey-900 btn-round-sm font-lg\'></i> {{count()}} Comment');});" onclick="viewComment('{{$post->Post_Id}}')" class="ms-auto d-flex align-items-center fw-600 text-grey-900 text-dark lh-26 font-xssss"><i class="feather-message-circle text-dark text-grey-900 btn-round-sm font-lg"></i>{{count($comments[$post->Post_Id])}} Comment</a>
            @else
